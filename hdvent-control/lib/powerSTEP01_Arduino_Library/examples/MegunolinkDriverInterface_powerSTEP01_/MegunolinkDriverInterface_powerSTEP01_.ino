...
powerSTEP Stepper(0, nCS_PIN, nSTBY_nRESET_PIN);
...
void setup() 
{
  ...
  // Configure powerSTEP
  Stepper.SPIPortConnect(&SPI); // give library the SPI port (only the one on an Uno)
  ...
  Stepper.setMaxSpeed(Config.Data.maxSpeed); // max speed in units of full steps/s
  Stepper.setFullSpeed(2000); // full steps/s threshold for disabling microstepping
  Stepper.setAcc(2000); // full steps/s^2 acceleration
  Stepper.setDec(2000); // full steps/s^2 deceleration
  
  Stepper.setSlewRate(SR_520V_us); // faster may give more torque (but also EM noise),
                                  // options are: 114, 220, 400, 520, 790, 980(V/us)
                                  
  Stepper.setOCThreshold(Config.Data.overCurrentThreshold);
  ...
  Stepper.setRunKVAL(Config.Data.runKval);
  Stepper.setAccKVAL(Config.Data.runKval);
  Stepper.setDecKVAL(Config.Data.runKval);
  Stepper.setHoldKVAL(Config.Data.holdKval);
...
{
  byte stepMode = Stepper.getStepMode();
...
void Cmd_GetStatus(CommandParameter &Parameters)
{
  ...
  Serial.println(((unsigned int)Stepper.getStatus()), HEX);
}
...
void Cmd_SetRunKVAL(CommandParameter &Parameters)
{
  ...
  Stepper.setRunKVAL(val);
  Stepper.setAccKVAL(val);
  Stepper.setDecKVAL(val);
...
void Cmd_GoUntil(CommandParameter &Parameters)
{
  ...
  float stepSpeed = MicrostepsToSteps(microStepSpeed);
  Stepper.goUntil(act, dir, stepSpeed);
