...
void setup() 
{
  ...
  Stepper.setMaxSpeed(1000); // max speed in units of full steps/s
...
{ 
  ...
  Stepper.move(FWD, 2000); // move forward 2000 microsteps
  while(Stepper.busyCheck()); // wait fo the move to finish
  Stepper.softStop(); // soft stops prevent errors in the next operation
  while(Stepper.busyCheck());
  
  Stepper.move(REV, 20000); // reverse back
  while(Stepper.busyCheck());
  Stepper.softStop();
  while(Stepper.busyCheck());
  
  Serial.println((int)Stepper.getStatus(), HEX);
