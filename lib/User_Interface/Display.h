...
public:
    ...
    Display(LiquidCrystal &lcd, Parameter_Container<UP_n, UP_t> &allUserParameters,
    ...
    void update(bool confirm, bool cancel, bool toggle, int8_t delta) {
        ...
        switch(_editState){
            ...
            case EDIT_ALARM: {
                ...
                if (toggle) {
                    param.setAlarmSettingChanged(true);
                    if (!((_navigationIndex - _mode->nParams) % 2)) {
    ...
    void printStaticText() {
        ...
        for (int i=0; i < (_mode->nParams); i++){
            ...
            if (_topRowIndex <= _cursorRow && _cursorRow< _topRowIndex+4) {
                setCursor(_cursorCol, _cursorRow);
                printValue(_allUserParams[_mode->parameters[i]].getDialValue());
            }
        }
